...
@router.get("/report/pl/year", response=response_schema, auth=AuthBearer())
def get_year_pl_report(request, farm_id: str, year: int):
    ...
    try:
        report = YearProfitLossService(farm_id, year).get_report()
