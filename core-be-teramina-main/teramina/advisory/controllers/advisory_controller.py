...
@router.get("/admin/assistant-brief-logs", response=response_schema, auth=AuthBearer())
def admin_list_assistant_brief_logs(request, case_id: str = "", status: str = "", limit: int = 50):
    user = get_signed_in_user(request)
    return AdvisoryService.admin_list_assistant_brief_logs(user, case_id=case_id, status=status, limit=limit)


@router.get("/admin/assistant-answer-logs", response=response_schema, auth=AuthBearer())
def admin_list_assistant_answer_logs(request, case_id: str = "", asked_by: str = "", limit: int = 50):
    user = get_signed_in_user(request)
    return AdvisoryService.admin_list_assistant_answer_logs(user, case_id=case_id, asked_by=asked_by, limit=limit)


@router.post("/admin/assistant-answer", response=response_schema, auth=AuthBearer())
def answer_assistant_question(request, data: AdvisoryAssistantAnswerSchema = Body(...)):
    user = get_signed_in_user(request)
    return AdvisoryService.answer_assistant_question(user, data)
...
def get_benchmark_consent(request, case_id: str):
    user = get_signed_in_user(request)
    return AdvisoryService.get_benchmark_consent(user, case_id)


@router.post("/cases/{case_id}/benchmark-consent", response=response_schema, auth=AuthBearer())
def accept_benchmark_consent(request, case_id: str, data: BenchmarkConsentSchema = Body(...)):
    user = get_signed_in_user(request)
    return AdvisoryService.accept_benchmark_consent(user, case_id, data)


@router.post("/cases/{case_id}/benchmark-consent/revoke", response=response_schema, auth=AuthBearer())
def revoke_benchmark_consent(request, case_id: str):
    user = get_signed_in_user(request)
    return AdvisoryService.revoke_benchmark_consent(user, case_id)
...
@router.get("/admin/report-workflow-events", response=response_schema, auth=AuthBearer())
def admin_list_report_workflow_events(request, report_id: str = "", case_id: str = "", limit: int = 50):
    user = get_signed_in_user(request)
    return AdvisoryService.admin_list_report_workflow_events(user, report_id=report_id, case_id=case_id, limit=limit)


@router.post("/admin/hatcheries", response=response_schema, auth=AuthBearer())
def create_hatchery_profile(request, data: HatcheryProfileSchema = Body(...)):
    user = get_signed_in_user(request)
    return AdvisoryService.create_hatchery_profile(user, data)


@router.get("/admin/hatcheries", response=response_schema, auth=AuthBearer())
def admin_list_hatchery_profiles(request, case_id: str = "", user_id: str = ""):
    user = get_signed_in_user(request)
    return AdvisoryService.admin_list_hatchery_profiles(user, case_id=case_id, user_id=user_id)


@router.patch("/admin/hatcheries/{hatchery_id}", response=response_schema, auth=AuthBearer())
def update_hatchery_profile(request, hatchery_id: str, data: HatcheryProfileUpdateSchema = Body(...)):
    user = get_signed_in_user(request)
    return AdvisoryService.update_hatchery_profile(user, hatchery_id, data)


@router.post("/admin/hatchery-records", response=response_schema, auth=AuthBearer())
def create_hatchery_record(request, data: HatcheryOperationalRecordSchema = Body(...)):
    user = get_signed_in_user(request)
    return AdvisoryService.create_hatchery_record(user, data)


@router.get("/admin/hatchery-records", response=response_schema, auth=AuthBearer())
def admin_list_hatchery_records(request, hatchery_id: str = "", case_id: str = "", record_type: str = ""):
    user = get_signed_in_user(request)
    return AdvisoryService.admin_list_hatchery_records(user, hatchery_id=hatchery_id, case_id=case_id, record_type=record_type)


@router.patch("/admin/hatchery-records/{record_id}", response=response_schema, auth=AuthBearer())
def update_hatchery_record(request, record_id: str, data: HatcheryOperationalRecordUpdateSchema = Body(...)):
    user = get_signed_in_user(request)
    return AdvisoryService.update_hatchery_record(user, record_id, data)


@router.post("/admin/investor-scores", response=response_schema, auth=AuthBearer())
def create_investor_due_diligence_score(request, data: InvestorDueDiligenceScoreSchema = Body(...)):
    user = get_signed_in_user(request)
    return AdvisoryService.create_investor_due_diligence_score(user, data)


@router.get("/admin/investor-scores", response=response_schema, auth=AuthBearer())
def admin_list_investor_due_diligence_scores(request, case_id: str = "", risk_level: str = ""):
    user = get_signed_in_user(request)
    return AdvisoryService.admin_list_investor_due_diligence_scores(user, case_id=case_id, risk_level=risk_level)


@router.patch("/admin/investor-scores/{score_id}", response=response_schema, auth=AuthBearer())
def update_investor_due_diligence_score(request, score_id: str, data: InvestorDueDiligenceScoreUpdateSchema = Body(...)):
    user = get_signed_in_user(request)
    return AdvisoryService.update_investor_due_diligence_score(user, score_id, data)


@router.post("/admin/investor-scores/{score_id}/report", response=response_schema, auth=AuthBearer())
def create_report_from_investor_score(request, score_id: str, data: InvestorDueDiligenceReportSchema = Body(...)):
    user = get_signed_in_user(request)
    return AdvisoryService.create_report_from_investor_score(user, score_id, data)


@router.get("/admin/phase-six-revisions", response=response_schema, auth=AuthBearer())
def admin_list_phase_six_revisions(request, record_kind: str = "", record_id: str = "", case_id: str = "", limit: int = 50):
    user = get_signed_in_user(request)
    return AdvisoryService.admin_list_phase_six_revisions(user, record_kind=record_kind, record_id=record_id, case_id=case_id, limit=limit)


@router.get("/admin/benchmarks/phase-six", response=response_schema, auth=AuthBearer())
def admin_phase_six_benchmarks(request, record_type: str = "", risk_level: str = "", project_type: str = "", from_month: str = "", to_month: str = ""):
    user = get_signed_in_user(request)
    return AdvisoryService.admin_phase_six_benchmarks(
        user,
