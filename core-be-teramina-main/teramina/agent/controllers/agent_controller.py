...
def _owns_context(user_id, farm_id="", pond_id="", cycle_id=""):
    return (
        (not farm_id or verify_farm_owner(farm_id, user_id))
...
def chat(request, data: ChatMessageSchema = Body(...)):
    ...
    if not _owns_context(str(user.id), data.farm_id, data.pond_id, data.cycle_id):
...
def get_control_loops(request, farm_id: str = "", cycle_id: str = "", include_closed: bool = False):
    user = get_signed_in_user(request)
    if not _owns_context(str(user.id), farm_id=farm_id, cycle_id=cycle_id):
        return _unauthorized()
...
def get_memories(request, farm_id: str = "", pond_id: str = "", limit: int = 20,
    ...
    user = get_signed_in_user(request)
    if not _owns_context(str(user.id), farm_id=farm_id, pond_id=pond_id):
        return _unauthorized()
...
def get_today_summary(request, farm_id: str):
    user = get_signed_in_user(request)
    if not _owns_context(str(user.id), farm_id=farm_id):
        return _unauthorized()
...
def get_pond_timeline(request, cycle_id: str, limit: int = 50):
    user = get_signed_in_user(request)
    if not _owns_context(str(user.id), cycle_id=cycle_id):
        return _unauthorized()
