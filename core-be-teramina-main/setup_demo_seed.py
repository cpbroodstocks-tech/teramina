...
setup_demo_seed.py
------------------
Run once to create the template farm/pond/cycle that every new user gets
cloned on first login.

Usage:
    cd core-be-teramina-main
    python setup_demo_seed.py
...
import mongoengine  # noqa: E402  (connected via Django settings)
...
POND_SIZE_M2      = 3000
INITIAL_STOCKING  = 360_000       # 120 PL/m²
START_DATE        = datetime(2024, 3, 1)
TOTAL_DAYS        = 120
PARTIAL_DOC       = 95            # partial harvest at DOC 95
PARTIAL_FRACTION  = 0.30          # 30% of remaining stock

# ABW interpolation checkpoints (DOC → g)
ABW_CHECKPOINTS = {
...
# Feed type by DOC range: (name, protein_pct, cost_idr_per_kg)
FEED_SCHEDULE = [
...
# Shrimp price by ABW (g) → IDR/kg
def shrimp_price(abw_g: float) -> float:
...
def generate_daily_data():
    ...
    # Cumulative survival rate schedule (DOC → cumulative mortality fraction)
    ...
    result_rows = []       # for CycleData.result_data (raw)
    feed_rows = []         # for FeedRealization
    cost_rows = []         # for CostData
    harvest_records = []   # for HarvestRecord

    population = INITIAL_STOCKING
    partial_harvested = 0
    cum_feed_kg = 0.0
    cum_cost_idr = 0.0
    ...
    for doc in range(1, TOTAL_DAYS + 1):
        ...
        # Feed
        feed_pct = max(2.5, 8.0 - 0.045 * doc)  # % of biomass
        ...
        other_day = 50_000 / 30
        total_cost_day = feed_cost_day + labor_cost_day + energy_cost_day + probiotic_day + other_day

        cum_feed_kg += feed_kg
        cum_cost_idr += total_cost_day
        fcr = cum_feed_kg / biomass_kg if biomass_kg > 0 else 0

        # Revenue (only on harvest days)
        ...
        if is_partial_harvest:
            ...
            realized_revenue = harvest_kg * price
            partial_harvested = harvest_kg
            harvest_records.append({
                "date": date_str,
                "doc": doc,
                "is_partial": True,
                "harvest_biomass_kg": harvest_kg,
                "abw_g": round(abw, 2),
                "sr_pct": round(sr_total * 100, 1),
                "bags": math.ceil(harvest_kg / 30),
                "buyer": "CV Mitra Udang Nusantara",
                "price_per_kg_idr": price,
                "notes": "Panen parsial 30%",
                "source": "seed",
            })

        if is_final_harvest:
            remaining_pop = round(population * (1 - PARTIAL_FRACTION) if doc == TOTAL_DAYS else population)
            ...
            harvest_records.append({
                ...
                "sr_pct": round(sr_total * 100, 1),
                "bags": math.ceil(harvest_kg / 30),
                "buyer": "PT Surya Bahari Sejahtera",
                "price_per_kg_idr": price,
                "notes": "Panen akhir",
                "source": "seed",
            })

        # ABW sampling (every 7 days from DOC 14)
        abw_entry = {}
        if doc >= 14 and (doc - 14) % 7 == 0:
            sample_n = 100
            total_w = round(abw * sample_n, 1)
            abw_entry = {
                "abw": round(abw, 2),
                "abw_sample_count": sample_n,
                "abw_total_weight_g": total_w,
                "abw_min_g": round(abw * 0.75, 2),
                "abw_max_g": round(abw * 1.28, 2),
        ...
        row = {
            "date": date_str,
            "doc": doc,
            "do_morning": round(do_m, 2),
            "do_afternoon": round(do_a, 2),
            "do_avg": round(do_avg, 2),
            "temp_morning": round(tmp_m, 2),
            "temp_afternoon": round(tmp_a, 2),
            "temp_avg": round(tmp_avg, 2),
            "ph_morning": round(ph_m, 2),
            "ph_afternoon": round(ph_a, 2),
            "salinity": round(sal, 1),
            "nh3": round(nh3, 4),
            "turbidity": round(turb, 1),
            "feed_given_kg": feed_kg,
            "feed_leftover": feed_leftover,
            "feed_type": fname,
            "protein_content": float(fprot),
            "feeding_frequency": freq,
            "mortality_count": daily_dead,
            "mortality_notes": "",
            "notes": "",
            "source": "seed",
            **abw_entry,
        ...
        # Monthly cost summary row (first day of each month bucket)
        if doc % 30 == 1:
    ...
    # Build ResultData (processed format used by dashboards)
    result_computed = []
    cum_feed = 0.0
    cum_cost = 0.0
    cum_rev = 0.0
    for i, row in enumerate(result_rows):
        ...
        sr_total = 1.0 - (0.04 * min(doc, 14) / 14 if doc <= 14 else (0.04 + 0.14 * min((doc - 14) / 46, 1) if doc <= 60 else (0.18 + 0.05 * min((doc - 60) / 35, 1) if doc <= 95 else (0.23 + 0.04 * min((doc - 95) / 25, 1)))))
        ...
        rev = 0.0
        for hr in harvest_records:
            if hr["doc"] == doc:
                rev = hr["harvest_biomass_kg"] * hr["price_per_kg_idr"]
        cum_feed += feed
        ...
        result_computed.append({
            ...
            "feed_given": feed,
            "total_cost": round(total_cost_d, 0),
            ...
            "labor_cost": labor_day,
            "bonus_cost": 0.0,
            "energy_cost": energy_rate,
            "probiotic_cost": probiotic_day,
            "other_cost": other_day,
            "harvest_cost": 0.0,
    ...
    return result_rows, result_computed, feed_rows, cost_rows, harvest_records


# ── Main ──────────────────────────────────────────────────────────────────────

def main():
    print("Generating 120-day demo data...")
    result_rows, result_computed, feed_rows, cost_rows, harvest_records = generate_daily_data()

    print(f"  Daily rows:     {len(result_rows)}")
    print(f"  Feed rows:      {len(feed_rows)}")
    print(f"  Cost entries:   {len(cost_rows)}")
    print(f"  Harvest events: {len(harvest_records)}")
    ...
    cycle = Cycle(
        ...
        is_active=False,  # template — not a real active cycle
    )
    cycle.save()
    cycle_id = str(cycle.id)
    print(f"Cycle created: {cycle_id}")

    # Update pond with active_cycle_id
    pond.active_cycle_id = cycle_id
    pond.save()

    # CycleData (raw daily data)
    CycleData(cycle_id=cycle_id, result_data=result_rows, last_updated=datetime.utcnow()).save()

    # ResultData (computed dashboard data)
    ResultData(cycle_id=cycle_id, result_data=result_computed, last_updated=datetime.utcnow()).save()

    # ForecastData (empty — will be computed on demand)
    ForecastData(cycle_id=cycle_id, result_data=[], last_updated=datetime.utcnow()).save()
    ...
    print(f"FeedRealization: {len(feed_rows)} records saved")
    ...
    print(f"HarvestRecord:   {len(harvest_records)} records saved")
    ...
    print(f"CostData:        {len(cost_rows)} entries saved")

    print("\n" + "=" * 60)
    print("Add these to your .env file:")
    print("=" * 60)
    print(f"SEEDER_FARM={farm_id}")
    print(f"SEEDER_POND={pond_id}")
    print(f"SEEDER_CYCLE={cycle_id}")
    print("=" * 60)
    print("\nDone. Every new user who signs in will get this data cloned automatically.")
