...
import { useStyles } from "features/cycle/modal-delete-cycle/styles";
import ConfirmDelete from "components/confirm-delete";
import { useDeleteCycle } from "features/cycle/queries";
import { useToastStore } from "store/toast.store";
import { useTranslation } from "react-i18next";

const ModalCycleDelete = ({ data }) => {
  const { t } = useTranslation();
  const { setToast: toast } = useToastStore();
  const { mutateAsync } = useDeleteCycle();
...
export default ModalCycleDelete;
